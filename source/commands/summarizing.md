...
- 避免冗余寒暄
- 洞察性总结放在最末
- **渐进式披露**（v2.1，仅外部大资料场景）：当本会话引用了外部资料（PDF / 长网页 / 论文 / 大 markdown / 用户原文等 > 屏内容）时，总结文档**只放指针 + 一句话摘要**；完整内容放 `references/`，避免总结文件膨胀占用上下文 token。**纯项目总结**（无外部资料）无需拆指针，8 维度直接写在总结文档里即可
